...
const Developer = ({ animationName = 'idle', ...props }) => {
  ...
  useEffect(() => {
    // Check if the action exists before trying to use it
    if (actions[animationName]) {
      actions[animationName].reset().fadeIn(0.5).play();
    }
    
    // Return cleanup function that also checks if action exists
    return () => {
      if (actions[animationName]) {
        actions[animationName].fadeOut(0.5);
      }
    };
  }, [animationName, actions]); // Added actions to dependency array
